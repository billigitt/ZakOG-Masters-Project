...
var_d = linspace(.3, -.6, Num_Serials); %Differences in means of old and new serials
...
for i = 1:Num_Serials
    ...
    [Delta_Matrix(1, :, i), ~] = Delta_Generator_Quadratic_1(w_o, 0, var_d(i)); %Generates Deltas with different means. Set differing variance to 5 as enables good plot
...
legend(h([1 2 3 4 5]),  strcat('$\mu_{\mbox{\boldmath $\Delta$}} \in ($', num2str(var_d(1), '%4.2f'),'$,$', num2str(var_d(20), '%4.2f'), '$)$'), strcat('$\mu_{\mbox{\boldmath $\Delta$}} \in ($', num2str(var_d(21), '%4.2f'),'$,$', num2str(var_d(40), '%4.2f'), '$)$'), strcat('$\mu_{\mbox{\boldmath $\Delta$}} \in ($', num2str(var_d(41), '%4.2f'),'$,$', num2str(var_d(60), '%4.2f'), '$)$'), strcat('$\mu_{\mbox{\boldmath $\Delta$}} \in ($', num2str(var_d(61), '%4.2f'),'$,$', num2str(var_d(80), '%4.2f'), '$)$'), strcat('$\mu_{\mbox{\boldmath $\Delta$}} \in ($', num2str(var_d(81), '%4.2f'),'$,$', num2str(var_d(100), '%4.2f'), '$)$'), 'Location', 'best')
...
saveas(gcf, 'Quadratic_Delta_Sigma.eps')
export_fig Quadratic_Delta_Sigma.eps -eps -r300 -painters -transparent
...
imagesc(R_t, var_d, Area_Dif.^(1/3))
colormap bone
colormap(f, flipud(colormap(f)))

% xlabel('True $R_t$')
ylabel("Variance of"+newline+"mis-specification, $\mathrm{Var}[\mbox{\boldmath $\Delta$}]$")
hYLabel = get(gca,'YLabel');
% set(hYLabel,'rotation',0,'VerticalAlignment','middle')
...
% xlabel('True $R_t$')
% ylabel("Variance of"+newline+" misspecification, $\mathrm{Var}[\mbox{\boldmath $\Delta$}]$")
% set(gca,'YDir','normal')
% hYLabel = get(gca,'YLabel');
% % set(hYLabel,'rotation',0,'VerticalAlignment','middle')
% hYLabel.Position(1) = -.7;
...
set(c,'TickLabelInterpreter','latex')
ylabel(c, '$\bigg( \frac{\int^{T_e}_{T_i} |\tilde{R}_t^o(s) - \tilde{R}_t^a(s)| \mathrm{d}s}{\int^{T_e}_{T_i} \tilde{R}_t^a(s) \mathrm{d}s} \bigg)^{1/3}$', 'Interpreter', 'latex', 'FontSize', 16)
c.Position(1) = 0.915;
%legend(h, {"Numerical solve for"+newline+"$v(R(r)) \cdot  \mu_{\Delta}=0$"}, 'Location', 'SouthEast')


%legend(h, {"Numerical solve for"+newline+"Numerical solve for"+newline+"$\mbox{\boldmath $v$}(R(r)) \cdot  \mbox{\boldmath $\Delta $}=0$"}, 'Location', 'SouthEast')
Printer = 1;
set(gca, 'FontSize', 14)
if Printer == 1
%Save figure
set(gcf, 'Units', 'centimeters', 'Position', [0 0 40 10], 'PaperUnits', 'centimeters', 'PaperSize', [40 10]);
saveas(gcf, 'Significant_Finding_Var_Sigma.pdf')
export_fig Significant_Finding_Var_Sigma.eps -pdf -r300 -painters -transparent
...
[X,Y] = meshgrid(R_t,var_d);
H = surf(X, Y, tanh(10*Area_Dif_NonAbs))
...
[X,Y] = meshgrid(R_t,var_d);
[c, h] = contourf(X, Y, tanh(200*(Ratio-1)), 'ShowText', 'off', 'LineColor', 'none');
% clabel(c,h, 'FontSize', 16, 'LabelSpacing', 20000, 'Interpreter', 'latex');
...
set(c,'TickLabelInterpreter','latex')
ylabel(c, '$\mathrm{tanh}(200(h(R_t)-1))$', 'Interpreter', 'latex', 'FontSize', 18)

% legend(h, 'Numerical solve for $0 = \mu_{\Delta} \cdot $\boldmath$v$ $_R$', 'Location', 'SouthEast')

%legend(h(1), {"Numerical solve for"+newline+"$v(R(r)) \cdot  \mu_{\Delta}=0$"}, 'Location', 'best')
...
[X,Y] = meshgrid(R_t,var_d);
[c, h(1)] = contourf(X, Y, Ratio, [0.96 0.97 0.98 0.99 0.995 1 1.005 1.01 1.02 1.03], 'ShowText', 'on', 'LineColor', 'none');
...
% h = plot3(R_star, mu_d, 'k', 'LineStyle', '--');
xlabel('True $R_t$')
ylabel("Variance of"+newline+" mis-specification")
hYLabel = get(gca,'YLabel');
hXLabel = get(gca, 'XLabel');
% set(hYLabel,'rotation',0,'VerticalAlignment','middle')
set(gca,'YDir','normal')
...
legend(h(1), "$h(R_t)$", 'Location', 'best', 'FontSize', 20)
% 
% t = title('Shape of $h(R_t)$ matches with mathematical analysis, for $\alpha=2$ and $\mu_{\mbox{\boldmath $\Delta$}}=0$')
% 
% t.Position(1) = 6.65;
...
saveas(gcf, 'Contour_Var_Sigma.eps')
export_fig Contour_Var_Sigma.eps -eps -r300 -painters -transparent
