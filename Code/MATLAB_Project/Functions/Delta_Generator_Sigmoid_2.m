function [K, Delta, F_final] = Delta_Generator_Sigmoid_2(N, mu_d, var_d)
...
F(1) = sum((k(1)*(i-k(2)))./(1+k(3)*(i-k(2)).^2));

F(2) = sum(i.*((k(1)*(i-k(2)))./(1+k(3)*(i-k(2)).^2)))-mu_d;

F(3) = sum(i.^2.*((k(1)*(i-k(2)))./(1+k(3)*(i-k(2)).^2)))-var_d;
    ...
    function Delta = Delta_From_K(y, n)
        ...
        for i = 1:n
            ...
            Delta(i) = (y(1)*(i-y(2)))./(1+y(3)*(i-y(2)).^2);
