...
writematrix(w_s_o,'Trivial_Serial.csv') %For comparison to EpiEstim
...
for t = 1:total_time
    ...
    I_new = poissrnd(R_t*Incidence_Generator_2(I, w_s_a));
...
%     disp(I(t-tau:t))
    ...
    Upper(t) = gaminv(0.975, Shape(t), Scale_a(t));
    
    Lower(t) = gaminv(0.025, Shape(t), Scale_a(t));
...
title({['Trivial $R_t$ (=', num2str(R_t), ') inference with'];...
    ['$w_s^o = [0$ ',num2str(w_s_o), '] \& $N_a = $',num2str(length(w_s_a)-1)]})
ylabel('$\bar{R}_t$')
xlabel('Time, $t$ (days)')

legend(h([1 2]), 'Actual', 'Original')
...
saveas(gcf, 'Poor_Serial_Trivial_Growth.eps')

export_fig Poor_Serial_Trivial_Growth.eps -eps -r300 -painters -transparent
...
%Save figure
set(gcf, 'Units', 'centimeters', 'Position', [0 0 20 15], 'PaperUnits', 'centimeters', 'PaperSize', [15 20]);
saveas(gcf, 'Poor_Serial_Trivial_Decay.eps')

export_fig Poor_Serial_Trivial_Decay.eps -eps -r300 -painters -transparent
